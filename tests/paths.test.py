...
def make_tree(base: Path, *, installed: bool) -> dict:
    ...
    后端子进程要沿路向上找到 resources 的上一级才是软件根目录。
    """
...
def main() -> int:
    ...
    try:
        ...
        ok("_install_dir：沿路向上找到 resources 的上一级",
        ...
        ok(
            ...
            paths._resolve_app_root(
                frozen=False, exe=portable["backend_exe"], local_app_data=str(fake_local),
        ...
        ok(
            ...
            paths._resolve_app_root(
                frozen=True, exe=portable["backend_exe"], local_app_data=str(fake_local),
                project_root=Path("/proj"),
            ) == p_root,
            str(paths._resolve_app_root(
                frozen=True, exe=portable["backend_exe"], local_app_data=str(fake_local),
                project_root=Path("/proj"),
            )),
        )
        root_installed = paths._resolve_app_root(
            frozen=True, exe=installed["backend_exe"], local_app_data=str(fake_local),
            project_root=Path("/proj"),
        )
        ok(
            "安装版：数据根目录 = %LOCALAPPDATA%/TeyvatMelody（升级卸载都不碰）",
            root_installed == (fake_local / paths.APP_DIR_NAME).resolve(),
            str(root_installed),
        )
        ok(
            "安装版：拿不到 LOCALAPPDATA 时退回安装目录（不崩、也能找到数据）",
            paths._resolve_app_root(
                frozen=True, exe=installed["backend_exe"], local_app_data=None,
                project_root=Path("/proj"),
            ) == i_root,
            str(paths._resolve_app_root(
                frozen=True, exe=installed["backend_exe"], local_app_data=None,
        ...
        ok(
            ...
            not str(root_installed).startswith(str(i_root)),
            str(root_installed),
        )
